...
void test_spring_sinusoid() {
  ...
  body_t *mass = body_init(make_square(), M, (rgb_color_t){0, 0, 0});
...
void test_energy_conservation() {
  ...
  const int STEPS = 1000000;
  scene_t *scene = scene_init();
  body_t *mass1 = body_init(make_square(), M1, (rgb_color_t){0, 0, 0});
  scene_add_body(scene, mass1);
  body_t *mass2 = body_init(make_square(), M2, (rgb_color_t){0, 0, 0});
...
void test_forces_removed() {
  scene_t *scene = scene_init();
  for (int i = 0; i < 10; i++) {
    body_t *body = body_init(make_square(), 1, (rgb_color_t){0, 0, 0});
