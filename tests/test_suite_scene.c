...
void test_force_creator() {
  ...
  scene_t *scene = scene_init();
  body_t *body = body_init(make_square(), 123, (rgb_color_t){0, 0, 0});
  vector_t radius = {R, 0};
...
void test_force_creator_aux() {
  ...
  scene_t *scene = scene_init();
  body_t *light = body_init(make_square(), LIGHT_MASS, (rgb_color_t){0, 0, 0});
  scene_add_body(scene, light);
  body_t *heavy = body_init(make_square(), HEAVY_MASS, (rgb_color_t){0, 0, 0});
...
void test_reaping() {
  scene_t *scene = scene_init();
  for (int i = 0; i < 3; i++) {
    scene_add_body(scene, body_init(make_square(), 1, (rgb_color_t){0, 0, 0}));
  }
